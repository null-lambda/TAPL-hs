...
tyRef :: Parser Ty -> Parser Ty
tyRef baseType = label "reference type" $ do
  reserved "Ref"
  TyRef <$> baseType
...
tyApp = choice [tyRef tyArr, tyArr]
...
tmRef :: Parser Term -> Parser Term
tmRef baseTerm = label "reference" $ reserved "ref" >> TmRef <$> baseTerm
...
tmAssign baseTerm = label "assignment" $ do
  t1 <- try (baseTerm <* symbol ":=")
  t2 <- baseTerm
...
expApp = (choice . map ($ expAsc))
  [tmUnaryFunc, tmBinaryFunc, tmRef, tmDeref, tmApp, id]
exp10 :: StateT Context (ParsecT Void String Identity) Term
exp10 = choice
  (  map ($ term)   [tmAbs, tmIf, tmLet, tmLetrec, tmCase]
  ++ map ($ expApp) [tmAssign, id]
  )
exp0 = exp10
term :: Parser Term
term = exp0 <?> "term"
